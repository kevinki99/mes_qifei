var model = avalon.define({
    ...
    SelModelXX: function () {
        model.FunData.clear();
        model.SelModel = null;
        model.FunData.pushArray([{ PageCode: "/BiManage/AppPage/XXZX/XXZXLIST", PageName: "消息管理", issel: true, isshow: true, order: 0 }]);
        model.ishasRight = false;
        model.selmenulev2(model.FunData[0]);
    },//选中最左侧事件
    SelUserInfo: function () {
        model.FunData.clear();
        model.SelModel = null;
        model.FunData.pushArray([{ PageCode: "/BiManage/AppPage/XTGL/UserCenter", PageName: "个人中心", issel: true, isshow: true, order: 0 }]);
        model.ishasRight = false;
        model.selmenulev2(model.FunData[0]);
    },//选中消息的事件
    selmenulev2: function (item, dom) {
        ...
        if (model.isiframe == 'Y') {
            ...
            if (clickTime != 'undefined' && (nowTime - clickTime < 1000) && dom) {
                ...
                model.PageCode = "/BiManage/Base/Loading";
                gomenu = function () {
    ...
    PageCode: "/BiManage/Base/Loading",//需要加载的模板
    rdm: ComFunJS.getnowdate('yyyy-mm-dd hh:mm'),//随机数
    Temprender: function () {
        if (typeof (tempindex) != "undefined" && model.PageCode != "/BiManage/Base/Loading") {
