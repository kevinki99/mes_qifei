...
namespace iMES.Custom.Controllers
{
    public partial class Base_ProcessLineController
    {
        ...
        public async Task<IActionResult> GetDetailRows(int ProcessLine_Id)
        {
            ...
            var rows = await _processLineListRepository.FindAsIQueryable(x => x.ProcessLine_Id == ProcessLine_Id, x => new Dictionary<object, QueryOrderBy>() {
                                { x.Sequence,QueryOrderBy.Asc }
                            })
                  .ToListAsync();
