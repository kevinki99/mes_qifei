      ...
      addRow() {
        ...
        let _rows = rows.map((row) => {
          return {
            ...
            WorkOrderCode: "待生成",
            ProcessLine_Id: row.Process_Id,
            Qty: 1,
            FinishQty: 0,
            UnitQty: 1,
            Status:1,
            BadQty:0,
            LevelPath:this.level + '.1'
          };
      });
      this.$emit('parentCall', ($parent) => {
       
        _rows.forEach(item1 => {
          let arr = $parent.$refs.detail.rowData;
        let nextArr =  arr.filter((item)=>{
          if(item.LevelPath.toString().indexOf('.')>=0)
          {
            return item.LevelPath.toString().split('.').length == (this.level.toString().split('.').length+1) && item.LevelPath.toString().indexOf(this.level.toString() + '.') == 0 
          }
          else
          {
            return 2 == (this.level.toString().split('.').length+1) && item.LevelPath.toString().indexOf(this.level.toString() + '.') == 0 
          }
        })
        if(nextArr.length>0)
        {
          let currentInt = 1;
          let front = nextArr[0].LevelPath.substring(0,nextArr[0].LevelPath.length - 1)
          nextArr.forEach(item2=>{
              let frontKey = parseInt(item2.LevelPath.substring(item2.LevelPath.length - 1))
              if(frontKey > currentInt)
              {
                currentInt = frontKey
              }
            })
          let currentValue = front + (currentInt + 1).toString();
          item1.LevelPath = currentValue;
        }
          $parent.$refs.detail.rowData.splice(this.index+1,0,item1);
        })
      });
        //关闭当前窗口
        this.model = false;
